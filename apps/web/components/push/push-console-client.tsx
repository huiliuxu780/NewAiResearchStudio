...
export type PushConsoleView = "overview" | "workbench" | "records";

export function PushConsoleClient({ view }: { view: PushConsoleView }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { mutate } = useSWRConfig();
  const [activeTab, setActiveTab] = useState(view === "workbench" ? "tasks" : "records");
  ...
  useEffect(() => {
    if (view === "workbench") {
      ...
      setActiveTab(nextTab === "channels" || nextTab === "tasks" || nextTab === "templates" ? nextTab : "tasks");

      const nextFocus = searchParams.get("focus");
      setFocusMode(nextFocus === "task-risk" || nextFocus === "channel-risk" || nextFocus === "template-risk" ? nextFocus : "none");

      const nextRiskFilter = searchParams.get("riskFilter");
      setTaskRiskFilter(
        nextRiskFilter === "risk" || nextRiskFilter === "failing" || nextRiskFilter === "dependency" ? nextRiskFilter : "all"
      );

      const dependencyType = searchParams.get("dependencyType");
      const dependencyId = searchParams.get("dependencyId");
      const dependencyName = searchParams.get("dependencyName");
      if ((dependencyType === "channel" || dependencyType === "template") && dependencyId) {
        setTaskDependencyFocus({ type: dependencyType, id: dependencyId });
        setTaskDependencyFocusName(dependencyName ?? null);
      } else {
      ...
      if (selectedTemplateId && templates.data?.items?.length) {
        const nextTemplate = templates.data.items.find((template) => template.id === selectedTemplateId) ?? null;
        if (nextTemplate) {
    ...
    if (view === "records") {
      ...
      const nextFocus = searchParams.get("focus");
      setFocusMode(nextFocus === "record-risk" ? "record-risk" : "none");

      const diagnostic = searchParams.get("diagnostic");
      setRecordDiagnosticFilter(
        diagnostic === "retryable" || diagnostic === "error-code" || diagnostic === "retry-exhausted" ? diagnostic : "all"
      );
      setRecordTaskFocus(
        searchParams.get("taskId")
          ? {
              id: searchParams.get("taskId")!,
              name: searchParams.get("taskName") ?? "关联任务",
            }
          : null
      );
      setRecordChannelFilter(searchParams.get("channelType") ?? "all");
      setRecordChannelIdFilter(searchParams.get("channelId") ?? "all");
      setRecordErrorCodeFilter(searchParams.get("errorCode") ?? "all");
      return;
    }

    setFocusMode("none");
  }, [view, searchParams, templates.data?.items]);
  ...
  function pushToRoute(pathname: string, params?: Record<string, string | null | undefined>) {
  ...
  function handleInspectTaskRecords(task: PushTask) {
    if (view !== "records") {
      pushToRoute("/push/records", { taskId: task.id, taskName: task.name });
      return;
    }
  ...
  function handleInspectChannelRecords(channel: PushChannel) {
    if (view !== "records") {
      pushToRoute("/push/records", { channelType: channel.channel_type, channelId: channel.id });
      return;
    }
  ...
  function handleInspectChannelTasks(channel: PushChannel) {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", {
        tab: "tasks",
        dependencyType: "channel",
        dependencyId: channel.id,
        dependencyName: channel.name,
      });
      return;
    }
  ...
  function handleInspectTemplateTasks(template: PushTemplate) {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", {
        ...
        dependencyId: template.id,
        dependencyName: template.name,
      });
      return;
    }
  ...
  function handleInspectChannelRisk() {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", { tab: "channels", focus: "channel-risk" });
      return;
    }
  ...
  function handleInspectTaskRisk(filter: PushTaskRiskFilter = "risk") {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", { tab: "tasks", focus: "task-risk", riskFilter: filter });
      return;
    }
  ...
  function handleInspectTemplateRisk() {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", { tab: "templates", focus: "template-risk" });
      return;
    }
  ...
  function handleOpenTemplatePreview(template: PushTemplate) {
    if (view !== "workbench") {
      pushToRoute("/push/workbench", { tab: "templates", templateId: template.id });
      return;
    }
  ...
  function handleInspectRecordRisk(filter: PushRecordDiagnosticFilter = "all") {
    if (view !== "records") {
      pushToRoute("/push/records", { focus: "record-risk", diagnostic: filter });
      return;
    }
  ...
  function renderPageHeader(title: string, description: string) {
    return (
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={view === "overview" ? "default" : "outline"} onClick={() => pushToRoute("/push/overview")}>
            推送监控
          </Button>
          <Button variant={view === "workbench" ? "default" : "outline"} onClick={() => pushToRoute("/push/workbench")}>
            推送工作台
          </Button>
          <Button variant={view === "records" ? "default" : "outline"} onClick={() => pushToRoute("/push/records")}>
            推送列表
          </Button>
        </div>
      </div>
  ...
  if (view === "overview") {
    return (
      <div className="space-y-6">
        {renderPageHeader("推送监控", "集中查看推送健康度、异常趋势和风险分布。")}
        {flashMessage && <PushFlashBanner message={flashMessage} />}

        <Card className="border-border/50 bg-muted/20 py-3">
          <CardContent className="grid gap-3 px-4 md:grid-cols-2">
            <div className="rounded-xl border border-border/60 bg-background/70 p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">最近 30 天成功率，直接反映整体推送质量。</p>
            </div>
            <div className="rounded-xl border border-border/60 bg-background/70 p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">合并待处理与重试中的记录，方便快速判断是否需要人工介入。</p>
            </div>
          </CardContent>
        </Card>
  ...
  if (view === "records") {
    return (
      <div className="space-y-6">
        {renderPageHeader("推送列表", "集中追踪执行记录、错误码和重试处置。")}
        {flashMessage && <PushFlashBanner message={flashMessage} />}
        ...
        <PushWorkbenchShortcuts
          ...
          onCreateChannel={() => pushToRoute("/push/workbench", { tab: "channels" })}
          onCreateTask={() => pushToRoute("/push/workbench", { tab: "tasks" })}
          onCreateTemplate={() => pushToRoute("/push/workbench", { tab: "templates" })}
          ...
          onPreviewSelectedTemplate={() => pushToRoute("/push/workbench", { tab: "templates" })}
        />
          ...
          <CardContent className="pt-4">
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">把错误码、重试、任务聚焦和同渠道诊断集中在一个页面里处理。</p>
              </div>
              {records.isLoading && (
  ...
  return (
    <div className="space-y-6">
      {renderPageHeader("推送工作台", "集中处理渠道、任务和模板，不再和监控、记录挤在一个页面。")}

      {flashMessage && <PushFlashBanner message={flashMessage} />}

      <PushWorkbenchShortcuts
        activeTab={activeTab as "channels" | "tasks" | "templates"}
        focusMode={focusMode}
        selectedTemplateName={selectedTemplate?.name ?? null}
        selectedRetryableCount={0}
        selectedRetryableTaskName={null}
        onCreateChannel={() => {
          setEditingChannel(null);
          setChannelDraftSource(null);
        ...
        <CardContent className="pt-4">
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value ?? "channels")} className="gap-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              {(channels.isLoading || tasks.isLoading || templates.isLoading) && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  同步最新推送数据
                </div>
